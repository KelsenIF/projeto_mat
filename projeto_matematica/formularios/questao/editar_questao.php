...
if (isset($_GET['id'])) {
    ...
    die("ID da questao não fornecido.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'editar_questao') {
    ...
    if (isset($_FILES['foto_quest']) && $_FILES['foto_quest']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['foto_quest']['tmp_name'], $file_path)) {
            ...
            if (!empty($foto_questao) && file_exists($foto_questao)) {
                unlink($foto_questao);
            }
    ...
    $sql_update = "UPDATE questões SET enunciado = ?, foto_questao = ?, alt_correta = ?, alt_incorreta1 = ?, alt_incorreta2 = ?, alt_incorreta3 = ?, origem = ?, id_nivel_ensino = ?, id_escolaridade = ?, id_assunto = ? WHERE id = ?";
    ...
    if ($stmt->rowCount()) {
        echo "<script>alert('questao atualizada com sucesso!'); window.location.href = 'questoes_em_analise.php';</script>";
    } else {
    ...
    <form class="#" action="#" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-3">
            ...
            <?php if (!empty($questao['foto_questao'])): ?>
                <img src="<?php echo htmlspecialchars($questao['foto_questao']); ?>" alt="Imagem da questao" style="max-width: 200px;">
        ...
        <div class="#">
            ...
            <select id="nivel_de_ensino" name="nivel_de_ensino" class="#" required>
                ...
                <?php foreach ($niveis_ensino as $nivel): ?>
                    <option value="<?php echo $nivel['id']; ?>" <?php echo ($nivel['id'] == $questao['id_nivel_ensino']) ? 'selected' : ''; ?>>
        ...
        <div>
            <button name="action" value="editar_questao" type="submit">SALVAR ALTERAÇÕES</button>
        </div>
