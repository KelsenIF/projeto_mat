...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'editar_questao') {
    ...
    $video_questao = $_POST['video_questao']; // NOVO CAMPO
    $material_questao = $_POST['material_questao']; // NOVO CAMPO
    ...
    if (isset($_FILES['foto_quest']) && $_FILES['foto_quest']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['foto_quest']['tmp_name'], $file_path)) {
            // Se houver uma foto antiga, exclua-a
    ...
    // Atualizar os dados no banco de dados com os novos campos
    $sql_update = "UPDATE questoes SET enunciado = ?, foto_questao = ?, video_questao = ?, material_questao = ?, alt_correta = ?, alt_incorreta1 = ?, alt_incorreta2 = ?, alt_incorreta3 = ?, origem = ?, id_nivel_ensino = ?, id_escolaridade = ?, id_assunto = ? WHERE id = ?";
