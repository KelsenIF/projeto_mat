...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'criar_questao') {
    ...
    $video_questao = ''; // Variável para armazenar o caminho do vídeo
    $material_questao = ''; // Variável para armazenar o caminho do material
    ...
    if (isset($_FILES['foto_quest']) && $_FILES['foto_quest']['error'] === UPLOAD_ERR_OK) {
        $file_name = uniqid('quest_img_') . '_' . basename($_FILES['foto_quest']['name']);
    ...
    // Lógica para lidar com o upload do vídeo
    if (isset($_FILES['video_quest']) && $_FILES['video_quest']['error'] === UPLOAD_ERR_OK) {
        $file_name = uniqid('quest_vid_') . '_' . basename($_FILES['video_quest']['name']);
        $file_path = $upload_dir . $file_name;

        if (move_uploaded_file($_FILES['video_quest']['tmp_name'], $file_path)) {
            $video_questao = $file_path;
        } else {
            die("Erro ao fazer o upload do vídeo.");
        }
    }

    // Lógica para lidar com o upload do material
    if (isset($_FILES['material_quest']) && $_FILES['material_quest']['error'] === UPLOAD_ERR_OK) {
        $file_name = uniqid('quest_mat_') . '_' . basename($_FILES['material_quest']['name']);
        $file_path = $upload_dir . $file_name;

        if (move_uploaded_file($_FILES['material_quest']['tmp_name'], $file_path)) {
            $material_questao = $file_path;
        } else {
            die("Erro ao fazer o upload do material.");
        }
    }

    // Preparar e executar a inserção no banco de dados
    $sql_insert = "INSERT INTO questoes (enunciado, foto_questao, video_questao, material_questao, alt_correta, alt_incorreta1, alt_incorreta2, alt_incorreta3, origem, id_nivel_ensino, id_escolaridade, id_assunto, id_situacao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    <div class="container mt-5">
        <div class="card shadow-lg p-4">
            ...
            <form class="needs-validation" action="#" method="POST" enctype="multipart/form-data" novalidate>
                ...
                <div class="mb-3">
                    <label for="video_quest" class="form-label">Vídeo da questão (opcional)</label>
                    <input class="form-control" type="file" id="video_quest" name="video_quest" accept="video/*">
                </div>

                <div class="mb-3">
                    <label for="material_quest" class="form-label">Material de apoio (PDF, DOCX, etc.) (opcional)</label>
                    <input class="form-control" type="file" id="material_quest" name="material_quest" accept=".pdf, .doc, .docx, .ppt, .pptx">
                </div>
