...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'criar_questão') {
    ...
    $foto_questão = ''; // Variável para armazenar o caminho da imagem
    ...
    $sql_insert = "INSERT INTO questões (enunciado, foto_questão, alt_correta, alt_incorreta1, alt_incorreta2, alt_incorreta3, origem, id_nível_ensino, id_escolaridade, id_disciplina, id_situacao) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
<body>
    <form class="#" action="#" method="POST" enctype="multipart/form-data">
        ...
        <div class="#">
            ...
            <select id="nível_de_ensino" name="nível_de_ensino" class="#" required>
                ...
                <?php foreach ($niveis_ensino as $ensinoW) { ?>
                    <option value="<?php echo $ensinoW['id'] ?>"><?php echo $ensinoW['nível_ensino'] ?></option>
                <?php } ?>
            </select>
        </div>
        ...
        <div>
            <button name="action" value="criar_questão" type="submit">CRIAR</button>
        </div>
