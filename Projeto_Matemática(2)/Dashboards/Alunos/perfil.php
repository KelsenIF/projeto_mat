...
// Use '../Include/connection.php' se o perfil.php estiver em uma subpasta (ex: Dashboards/Profile)
require_once '..\Include\connection.php'; // **MANTIDO O CAMINHO ORIGINAL, VERIFIQUE SE ESTÁ CORRETO**
...
try {
    // CORREÇÃO:
    // 1. Alterado 'e.Nome' para 'e.nome_escola'
    // 2. Alterado 'e.Município' para 'e.cidade'
    // 3. Alterado o LEFT JOIN de 'e.Código' para 'e.cod_inep'
    $sql = "
        SELECT 
            c.nome, 
            c.user, 
        ...
        FROM 
            cadastros c
        LEFT JOIN 
            escolas e ON c.instituicao = e.cod_inep
        LEFT JOIN 
    ...
    if (!$usuario) {
        // Esta exceção será ativada se o $_SESSION['user_id'] não existir em 'cadastros'
        throw new Exception("Usuário não encontrado.");
    }

} catch (PDOException $e) {
    error_log("Erro ao buscar dados do perfil: " . $e->getMessage());
    $error_message = "Não foi possível carregar as informações do perfil. (Erro de Banco de Dados)";
    $usuario = null;
} catch (Exception $e) {
